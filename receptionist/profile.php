 ...
 * Receptionist can change password, name, email, and delete account
 */
...
try {
    ...
    // Get profile picture
    $stmt = $conn->prepare("SELECT ProfilePicture FROM USER_PROFILE WHERE UserId = ? AND UserRole = 'receptionist'");
    $stmt->execute([$receptionist_id]);
    ...
    if ($action === 'upload_picture') {
        if (isset($_FILES['profile_picture']) && $_FILES['profile_picture']['error'] === UPLOAD_ERR_OK) {
            ...
            if (!in_array($file_ext, $allowed_extensions)) {
                ...
                $unique_name = 'receptionist_' . $receptionist_id . '_' . time() . '.' . $file_ext;
                ...
                if (move_uploaded_file($file['tmp_name'], $file_path)) {
                    try {
                        ...
                        $stmt = $conn->prepare("INSERT INTO USER_PROFILE (UserId, UserRole, ProfilePicture) VALUES (?, 'receptionist', ?) 
                                                ON DUPLICATE KEY UPDATE ProfilePicture = ?");
                        $stmt->execute([$receptionist_id, $file_path, $file_path]);
        ...
        if (empty($firstname) || empty($lastname) || empty($phone) || empty($email)) {
            ...
            try {
                $stmt = $conn->prepare("UPDATE RECEPTIONIST SET FirstName = ?, LastName = ?, PhoneNo = ?, Email = ? WHERE RecId = ?");
                $stmt->execute([$firstname, $lastname, $phone, $email, $receptionist_id]);
        ...
        if (empty($current_password) || empty($new_password) || empty($confirm_password)) {
            ...
            if (!empty($password_errors)) {
                ...
                try {
                    ...
                    if (password_verify($current_password, $receptionist['Password'])) {
                        $hashed_password = password_hash($new_password, PASSWORD_DEFAULT);
                        $stmt = $conn->prepare("UPDATE RECEPTIONIST SET Password = ? WHERE RecId = ?");
                        $stmt->execute([$hashed_password, $receptionist_id]);
        ...
        if (empty($security_question)) {
        ...
        } elseif (empty($security_answer) && empty($receptionist['SecurityAnswer'])) {
            ...
            try {
                if (!empty($security_answer)) {
                    ...
                    $stmt->execute([$security_question, strtolower(trim($security_answer)), $receptionist_id]);
                } else {
                    $stmt = $conn->prepare("UPDATE RECEPTIONIST SET SecurityQuestion = ? WHERE RecId = ?");
                    $stmt->execute([$security_question, $receptionist_id]);
                }
    ...
    <form method="POST" action="">
        ...
        <div class="form-group">
            ...
            <input type="text" id="firstname" name="firstname" 
                   value="<?php echo htmlspecialchars($receptionist['FirstName'] ?? ''); ?>" required>
        </div>
        
        <div class="form-group">
            <label for="lastname">Last Name *</label>
            <input type="text" id="lastname" name="lastname" 
                   value="<?php echo htmlspecialchars($receptionist['LastName'] ?? ''); ?>" required>
        </div>
        
        <div class="form-group">
            <label for="phone">Phone Number *</label>
            <input type="text" id="phone" name="phone" 
                   value="<?php echo htmlspecialchars($receptionist['PhoneNo'] ?? ''); ?>" required>
        </div>
        ...
        <div class="form-group">
            ...
            <input type="text" value="<?php echo htmlspecialchars($receptionist['Username'] ?? ''); ?>" disabled>
...
<div class="form-container">
    ...
    <form method="POST" action="">
            ...
            <select id="security_question" name="security_question" required>
                ...
                <?php foreach ($security_questions as $question): ?>
                    <option value="<?php echo htmlspecialchars($question); ?>" 
                            <?php echo (isset($receptionist['SecurityQuestion']) && $receptionist['SecurityQuestion'] === $question) ? 'selected' : ''; ?>>
        ...
        <div class="form-group">
            <label for="security_answer">Security Answer <?php echo empty($receptionist['SecurityAnswer']) ? '*' : ''; ?></label>
            <input type="text" id="security_answer" name="security_answer" 
                   value="" 
                   <?php echo empty($receptionist['SecurityAnswer']) ? 'required' : ''; ?> 
                   placeholder="Enter your answer">
            <?php if (!empty($receptionist['SecurityAnswer'])): ?>
                <small style="color: var(--gray); font-size: 12px; display: block; margin-top: 5px;">
